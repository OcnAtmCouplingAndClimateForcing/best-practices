---
title: "Univariate time series examples - best practices - real data examples"
author: "Eric Ward"
date: "`r Sys.Date()`"
output:
  pdf_document:
...
Here are examples of univariate times series models applied to smoothed winter sst and the shared trend for catch of four spp of salmon.

```{r, fig.pos="placeHere",fig.cap="Time series of smoothed winter SST and the shared trend for catch of four spp of salmon."}
dat=read.csv('~/best-practices/data/example 1 - salmon catch and sst.csv')
dat$era <- ifelse(dat$year<1990, "pre", "post")
names(dat)[1:3]= c('time','y','x')

g1 = ggplot2::ggplot(dat,aes(time,x)) + geom_point() + 
  xlab("Time") + ylab("Winter SST")
g2 = ggplot2::ggplot(dat,aes(time,y)) + geom_point() + 
  xlab("Time") + ylab("Shared trend for salmon catches")
